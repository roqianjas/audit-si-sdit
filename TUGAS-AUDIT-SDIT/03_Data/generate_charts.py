...
SD IT Al-huda Kelapa Gading — COBIT 2019
...
# === PATHS ===
BASE = "/www/wwwroot/audit-si-sdit/TUGAS-AUDIT-SDIT/03_Data"
...
# === DATA ===
domains = ['APO07', 'BAI09', 'DSS01', 'DSS03', 'DSS05']
domain_names = [
    'Managed\nHR',
    'Managed\nAssets',
    'Managed\nOperations',
...
# Capability data (calculated from master excel data)
l1_pct = [62.50, 65.83, 65.83, 65.83, 67.50]
l2_pct = [52.50, 55.00, 56.67, 56.67, 52.50]
l3_pct = [44.17, 45.83, 46.67, 46.67, 44.17]
cap_level = [2, 2, 2, 2, 2]
...
def chart_radar():
    ...
    plt.title('Radar Chart Capability Level\n5 Domain COBIT 2019 (SD IT Al-huda)', pad=25, fontsize=14, fontweight='bold', color=C_NAVY)
    plt.tight_layout()
...
def chart_metodologi():
    ...
    colors = [C_NAVY, C_BLUE, C_TEAL, '#2E7D32', C_ORANGE, C_RED, C_GOLD]
    box_w = 2.8
    box_h = 1.0
    start_x = 0.5

    # Title
    ax.text(5, 9.5, 'Alur Metodologi Penelitian', fontsize=16, fontweight='bold',
            color=C_NAVY, ha='center', va='center')
    ax.text(5, 9.0, 'Framework COBIT 2019', fontsize=12, color=C_GRAY,
            ha='center', va='center')

    # Draw steps in 2 rows
    positions = []
    ...
    for i in range(4):
        ...
        # Step number
        ax.text(x + 0.25, y + box_h/2 - 0.15, str(i+1),
                fontsize=10, fontweight='bold', color='white',
                ha='center', va='center',
                bbox=dict(boxstyle='circle', facecolor='white', edgecolor=colors[i], alpha=0.9))

        ax.text(x + box_w/2, y, steps[i][0], fontsize=8, fontweight='bold',
                color='white', ha='center', va='center', linespacing=1.3)

    # Arrow from step 4 going down
    ax.annotate('', xy=(start_x + 3*2.3 + box_w/2, 5.5 + box_h/2),
                xytext=(start_x + 3*2.3 + box_w/2, 7.0 - box_h/2),
                arrowprops=dict(arrowstyle='->', color=C_GRAY, lw=2.5))

    # Row 2: Steps 5-7 (right to left)
    for i in range(3):
        idx = 4 + i
        x = start_x + (3 - i) * 2.3  # reverse direction
        y = 4.5

        rect = FancyBboxPatch((x, y - box_h/2), box_w, box_h,
                              boxstyle="round,pad=0.15",
                              facecolor=colors[idx], edgecolor='white', linewidth=2)
        ax.add_patch(rect)

        # Step number
        ax.text(x + 0.25, y + box_h/2 - 0.15, str(idx+1),
                fontsize=10, fontweight='bold', color='white',
                ha='center', va='center',
                bbox=dict(boxstyle='circle', facecolor='white', edgecolor=colors[idx], alpha=0.9))

        ax.text(x + box_w/2, y, steps[idx][0], fontsize=8, fontweight='bold',
                color='white', ha='center', va='center', linespacing=1.3)

    # Arrows Row 1 (left to right)
    for i in range(3):
        x1 = start_x + i * 2.3 + box_w
        x2 = start_x + (i+1) * 2.3
        y = 7.0
    ...
    for i in range(2):
        x1 = start_x + (3-i) * 2.3  # right box left edge
        x2 = start_x + (2-i) * 2.3 + box_w  # left box right edge
        y = 4.5
        ax.annotate('', xy=(x2, y), xytext=(x1, y),
                    arrowprops=dict(arrowstyle='<-', color=C_GRAY, lw=2.5))
    ...
    output_x = 2.5
    output_y = 2.2
    rect = FancyBboxPatch((output_x, output_y - 0.5), 5, 1.0,
                          boxstyle="round,pad=0.2",
                          facecolor=C_LIGHT, edgecolor=C_NAVY, linewidth=2)
    ax.add_patch(rect)
    ax.text(output_x + 2.5, output_y + 0.15, '📋 OUTPUT', fontsize=11,
            fontweight='bold', color=C_NAVY, ha='center', va='center')
    ax.text(output_x + 2.5, output_y - 0.2, 'Laporan Audit SI dengan Capability Level & Rekomendasi',
            fontsize=9, color=C_GRAY, ha='center', va='center')

    # Arrow from step 7 to output
    ax.annotate('', xy=(output_x + 2.5, output_y + 0.5),
                xytext=(start_x + 1*2.3 + box_w/2, 4.5 - box_h/2),
...
# ============================================================
# GAMBAR 5: STRUKTUR ORGANISASI SD IT AL-HUDA
# ============================================================
def chart_struktur_organisasi():
    ...
    ax.text(10.5, 11.3, 'Struktur Organisasi', fontsize=16, fontweight='bold',
            color=C_NAVY, ha='center', va='center')
    ax.text(10.5, 10.8, 'SD IT Al-huda Kelapa Gading', fontsize=12, color=C_GRAY,
            ha='center', va='center')
    ...
    # === Level 1: Kepala Yayasan / Kepala Sekolah ===
    org_box(10.5, 9.8, 3.2, 0.7, 'KEPALA SEKOLAH', C_NAVY, 10)
    ...
    conn_down(10.5, 9.8 - 0.35, 8.8)
    ax.plot([4.5, 16.5], [8.8, 8.8], color='#999999', lw=1.5)

    for dx, name, color in depts:
        conn_down(dx, 8.8, 8.2 + 0.35)
        org_box(dx, 8.2, 2.5, 0.7, name, color, 9)

    # === Level 3: Bawah TU ===
    tu_subs = [
        (10.5, 'Operator\nSekolah', C_TEAL),
        (12.5, 'Bendahara\nBOSP', C_TEAL),
        (14.5, 'Pustakawan', C_TEAL),
    ]
    conn_down(12.5, 8.2 - 0.35, 7.0)
    ax.plot([10.5, 14.5], [7.0, 7.0], color='#999999', lw=1.5)

    for sx, name, color in tu_subs:
        conn_down(sx, 7.0, 6.4 + 0.35)
        org_box(sx, 6.4, 1.8, 0.7, name, color, 8)

    # === Level 3: Bawah Waka Kurikulum ===
    guru = [
        (3.5, 'Guru\nWali Kelas', C_TEAL),
        (5.5, 'Guru\nTIK / Lab', C_TEAL),
    ]
    conn_down(4.5, 8.2 - 0.35, 7.0)
    ax.plot([3.5, 5.5], [7.0, 7.0], color='#999999', lw=1.5)

    for sx, name, color in guru:
        conn_down(sx, 7.0, 6.4 + 0.35)
        org_box(sx, 6.4, 1.8, 0.7, name, color, 8)
    ...
    ax.add_patch(rect_b)
    ax.text(1.3, 3.85, '= Fokus Audit Utama', fontsize=8, color=C_NAVY, ha='left')
    rect_g = FancyBboxPatch((3.0, 3.7), 0.4, 0.3, boxstyle="round,pad=0.05",
                            facecolor=C_TEAL, edgecolor='white')
    ax.add_patch(rect_g)
    ax.text(3.7, 3.85, '= Responden Terkait', fontsize=8, color=C_NAVY, ha='left')
...
if __name__ == '__main__':
    print("=" * 60)
    print("GENERATING CHARTS — Laporan Audit SI SD IT Al-huda")
    print("=" * 60)
